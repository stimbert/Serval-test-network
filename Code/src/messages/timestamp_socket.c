...
int saw_timestamp(char *sender_prefix, int stratum, struct timeval *tv)
{
  ...
  if (debug_radio)
  {
     ...
     */
    snprintf(socketMessage, BUFFERSIZE, "Saw timestamp from %s: stratum=0x%02x, our stratum=0x%02x.0x%02x\n",
             sender_prefix, stratum, my_time_stratum >> 8, my_time_stratum & 0xff);
  ...
  if (gettime_ms() > next_time_update_allowed_after)
  {
    if ((stratum < (my_time_stratum >> 8)))
    {
      ...
      if (time_slave && (!monitor_mode))
      {
        ...
        if (last_message_update_time)
          last_message_update_time += delta;
        if (congestion_update_time)
        ...
        if (status_dump_epoch)
          status_dump_epoch += delta;
        if (last_servald_contact)
          last_servald_contact += delta;
	...
	printf("Saw timestamp from %s with stratum 0x%02x,"
	       ...
	       sender_prefix,stratum,my_time_stratum>>8,my_time_stratum&0xff);
        */
        snprintf(socketMessage, BUFFERSIZE, "Saw timestamp from %s with stratum 0x%02x,"
                                             " which is better than our stratum of 0x%02x.0x%02x\n",
                 sender_prefix, stratum, my_time_stratum >> 8, my_time_stratum & 0xff);
        send_all(socketMessage);
      }
                      ...
                      unsigned char *msg, int length)
{
  ...
  int offset = 0;
  {
    ...
    */
    ...
    if (delta > 60)
    {
      ...
      if (debug_bundlelog)
      {
        ...
        if (bundlelogfile)
        {
          ...
          snprintf(socketMessage, BUFFERSIZE, "Logged timestamp message.\n");
          send_all(socketMessage);
          fclose(bundlelogfile);
        }
        else
          perror("Could not open bundle log file");
      }
      else
      {
        //fprintf(stderr, "Logging timestamps disabled via debug_bundlelog.\n");
        snprintf(socketMessage, BUFFERSIZE, "Logging timestamps disabled via debug_bundlelog.\n");
        send_all(socketMessage);
      }
    }
    else
    {
      /*
      fprintf(stderr, "Not logging timestamp message, since we logged one just recently (%lld-%lld = %lld).\n",
              (long long)now, (long long)sender->last_timestamp_received,
              (long long)now - (long long)sender->last_timestamp_received);
              */
      snprintf(socketMessage, BUFFERSIZE, "Not logging timestamp message, since we logged one just recently (%lld-%lld = %lld).\n",
               (long long)now, (long long)sender->last_timestamp_received,
               (long long)now - (long long)sender->last_timestamp_received);
      send_all(socketMessage);
    }
