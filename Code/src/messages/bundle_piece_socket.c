                                  ...
                                  int target_peer)
{
  ...
  snprintf(socketMessage, BUFFERSIZE, ">>> %s I just sent %s piece [%d,%d) for %s*.\n",
           timestamp_str(), is_manifest ? "manifest" : "body",
           start_offset, start_offset + actual_bytes,
           peer_records[target_peer]->sid_prefix);
  ...
  for (int i = 0; i < 8; i++)
    msg[(*offset)++] = bundles[bundle_number].bid_bin[i];
  ...
  for (int pn = 0; pn < peer_count; pn++)
  {
    if (pn != target_peer && peer_records[pn])
    {
      if (peer_records[pn]->tx_bundle == bundle_number)
      {
        if (is_manifest)
        {
          if ((peer_records[pn]->tx_bundle_manifest_offset >= start_offset) && (peer_records[pn]->tx_bundle_manifest_offset < (start_offset + actual_bytes)))
            peer_records[pn]->tx_bundle_manifest_offset = (start_offset + actual_bytes);
        }
        else
        {
          if ((peer_records[pn]->tx_bundle_body_offset >= start_offset) && (peer_records[pn]->tx_bundle_body_offset < (start_offset + actual_bytes)))
          {
            if (debug_pieces)
            {
              /*
        ...
        */
              snprintf(socketMessage, BUFFERSIZE, ">>> %s Cursor advance from %d to %d, due to sending [%d..%d].\n",
                       timestamp_str(),
                       peer_records[pn]->tx_bundle_body_offset, (start_offset + actual_bytes),
                       start_offset, (start_offset + actual_bytes));
  ...
  if (debug_announce)
  {
    ...
    printf("T+%lldms : Announcing for %s* ", gettime_ms() - start_time,
           peer_records[target_peer]->sid_prefix);
    */
    snprintf(socketMessage, BUFFERSIZE, "T+%lldms : Announcing for %s* ", gettime_ms() - start_time,
             peer_records[target_peer]->sid_prefix);
    send_all(socketMessage);
    for (int i = 0; i < 8; i++)
      snprintf(socketMessage, BUFFERSIZE, "%c* (priority=0x%llx) version %lld %s segment [%d,%d)\n",
               bundles[bundle_number].bid_hex[i],
    ...
    send_all(socketMessage);
    /*
    printf("%c", bundles[bundle_number].bid_hex[i]);
...
char socketMessage[BUFFERSIZE];
  ...
  if (peer < 0)
  {
    ...
    */
    snprintf(socketMessage, BUFFERSIZE, ">>> %s Saw a piece from unknown SID=%s* -- ignoring.\n",
             timestamp_str(), peer_prefix);
    send_all(socketMessage);
    return -1;
  }
  ...
  if (debug_pieces)
    ...
    */
    snprintf(socketMessage, BUFFERSIZE, ">>> %s Saw a piece of BID=%s* from SID=%s*: %s [%lld,%lld) %s\n",
             timestamp_str(), bid_prefix, peer_prefix,
             is_manifest_piece ? "manifest" : "body",
             piece_offset, piece_offset + piece_bytes,
             is_end_piece ? "END PIECE" : "");
  ...
  if (for_me)
  {
    if (sync_is_bundle_recently_received(bid_prefix, version))
    {
      ...
      fprintf(stderr,
              ...
              bid_prefix, version);
      */
      snprintf(socketMessage, BUFFERSIZE, "We recently received %s* version %lld - ignoring piece.\n",
               bid_prefix, version);
      send_all(socketMessage);
  ...
  for (int i = 0; i < bundle_count; i++)
  {
    if (!strncasecmp(bid_prefix, bundles[i].bid_hex, strlen(bid_prefix)))
    {
      if (debug_pieces)
        /*
        printf("We have version %lld of BID=%s*.  %s is offering us version %lld\n",
               bundles[i].version, bid_prefix, peer_prefix, version);
        */
        snprintf(socketMessage, BUFFERSIZE, "We have version %lld of BID=%s*.  %s is offering us version %lld\n",
                 bundles[i].version, bid_prefix, peer_prefix, version);
      send_all(socketMessage);
      if (version <= bundles[i].version)
      {
        ...
        if (bundle_number >= 0)
        {
          ...
          printf(">>> %s Examining transmitted piece for bitmap updates.\n",
                 timestamp_str());
          */
          snprintf(socketMessage, BUFFERSIZE, ">>> %s Examining transmitted piece for bitmap updates.\n",
                   timestamp_str());
          send_all(socketMessage);
  ...
  for (i = 0; i < MAX_BUNDLES_IN_FLIGHT; i++)
  {
    ...
    else
    {
      if (!strcasecmp(partials[i].bid_prefix, bid_prefix))
      {
        ...
        {
          /*
          printf("Saw another piece for BID=%s* from SID=%s: ",
                 bid_prefix, peer_prefix);
        */
          snprintf(socketMessage, BUFFERSIZE, "Saw another piece for BID=%s* from SID=%s: ",
                   bid_prefix, peer_prefix);
          send_all(socketMessage);
        }
        if (debug_pieces)
        {
          ...
          printf("[%lld..%lld)\n",
                 piece_offset, piece_offset + piece_bytes);
          */
          snprintf(socketMessage, BUFFERSIZE, "[%lld..%lld)\n",
                   piece_offset, piece_offset + piece_bytes);
          send_all(socketMessage);
        }
      ...
      else
      {
        if (debug_pieces)
        {
          ...
          */
          snprintf(socketMessage, BUFFERSIZE, "  this isn't the partial we are looking for.\n  piece is of %s*, but slot #%d has %s*\n",
                   bid_prefix, i,
                   partials[i].bid_prefix);
          send_all(socketMessage);
        }
  ...
  if (debug_pieces)
  {
    ...
    snprintf(socketMessage, BUFFERSIZE, "Saw a piece of interesting bundle BID=%s*/%lld from SID=%s\n",
             bid_prefix, version, peer_prefix);
    send_all(socketMessage);
  }
  ...
  if (i == MAX_BUNDLES_IN_FLIGHT)
  {
    ...
    if (debug_pieces)
    {
      ...
      snprintf(socketMessage, BUFFERSIZE, "Didn't find bundle in partials for this peer. first spare slot =%d\n", spare_record);
      send_all(socketMessage);
    }
    ...
    {
      // printf("@@@   Using slot %d\n", i);
      snprintf(socketMessage, BUFFERSIZE, "@@@   Using slot %d\n", i);
      send_all(socketMessage);
    }
  ...
  if ((bundle_number > -1) && (!partials[i].body_segments))
  {
    ...
    snprintf(socketMessage, BUFFERSIZE, "%s:%d:My SID as hex is %s\n", __FILE__, __LINE__, my_sid_hex);
    send_all(socketMessage);
    ...
    if (!prime_bundle_cache(bundle_number, my_sid_hex, servald_server, credential))
    {
      ...
      if (debug_pieces)
      {
        ...
        printf("Preloaded %d bytes from old version of journal bundle.\n",
               cached_body_len);
        */
        snprintf(socketMessage, BUFFERSIZE, "Preloaded %d bytes from old version of journal bundle.\n",
                 cached_body_len);
        send_all(socketMessage);
      }
    }
    else
    {
      if (debug_pieces)
      {
        ...
        snprintf(socketMessage, BUFFERSIZE, "Failed to preload bytes from old version of journal bundle. XFER will likely fail due to far end thinking it can skip the bytes we already have, so ignoring current piece.\n");
        send_all(socketMessage);
      }
      return -1;
    }
  ...
  if (is_manifest_piece)
    s = &partials[i].manifest_segments;
  else
    s = &partials[i].body_segments;
  ...
  */
  ...
  while (1)
  {
    ...
    if ((!(*s)) || (segment_end < piece_offset))
    {
      ...
      {
        /*
        printf("Inserting piece [%lld..%lld) before [%d..%d)\n",
               piece_offset, piece_offset + piece_bytes,
               segment_start, segment_end);
        */
        snprintf(socketMessage, BUFFERSIZE, "Inserting piece [%lld..%lld) before [%d..%d)\n",
                 piece_offset, piece_offset + piece_bytes,
                 segment_start, segment_end);
        send_all(socketMessage);
      }
    ...
    else if (piece_end < segment_start)
    {
      ...
      if (debug_pieces)
      {
        ...
        printf("Piece [%lld..%lld) comes before [%d..%d)\n",
               ...
               segment_start, segment_end);
        */
        snprintf(socketMessage, BUFFERSIZE, "Piece [%lld..%lld) comes before [%d..%d)\n",
                 piece_offset, piece_offset + piece_bytes,
                 segment_start, segment_end);
        send_all(socketMessage);
      }
    ...
    else
    {
      ...
      assert(((segment_start >= piece_start) && (segment_start <= piece_end)) || ((segment_end >= piece_start) && (segment_end <= piece_end)));
      if (0)
      {
  ...
  snprintf(socketMessage, BUFFERSIZE, "(Piece was [%lld,%lld)\n", piece_offset, piece_offset + piece_bytes);
  send_all(socketMessage);
  ...
  if (partials[i].manifest_segments && partials[i].body_segments && (!partials[i].manifest_segments->next) && (!partials[i].body_segments->next) && (partials[i].manifest_segments->start_offset == 0) && (partials[i].body_segments->start_offset == 0) && (partials[i].manifest_segments->length == partials[i].manifest_length) && (partials[i].body_segments->length == partials[i].body_length))
  {
    ...
    snprintf(socketMessage, BUFFERSIZE, ">>> %s We have the entire bundle %s*/%lld now.\n",
             timestamp_str(), bid_prefix, version);
    send_all(socketMessage);
    ...
    if (!manifest_binary_to_text(partials[i].manifest_segments->data,
                                 ...
                                 manifest, &manifest_len))
    {
      ...
      if (debug_bundlelog)
      {
        ...
        snprintf(socketMessage, BUFFERSIZE, "Logging new bundle...\n");
        send_all(socketMessage);
      ...
      // but it's a start.
      sync_remember_recently_received_bundle(partials[i].bid_prefix,
    ...
    else
    {
      ...
      printf(">>> %s Could not decompress binary manifest.  Not inserting\n",
             timestamp_str());
      */
      snprintf(socketMessage, BUFFERSIZE, ">>> %s Could not decompress binary manifest.  Not inserting\n",
             timestamp_str());
      send_all(socketMessage);
    ...
    if (insert_result)
    {
      ...
      snprintf(socketMessage, BUFFERSIZE, "Failed to insert bundle %s*/%lld (result=%d)\n",
              partials[i].bid_prefix,
              partials[i].bundle_version, insert_result);
      send_all(socketMessage);              
  ...
  else
  {
    ...
    else
    {
      ...
      snprintf(socketMessage, BUFFERSIZE, "Sending BITMAP\n");
      send_all(socketMessage);
                      ...
                      unsigned char *msg, int length)
{
  ...
  if ((my_sid[0] != msg[offset]) || (my_sid[1] != msg[offset + 1]))
    for_me = 0;
  else
    for_me = 1;
  offset += 2;
