                          ...
                          char *servald_server, char *credential)
{
  ...
  snprintf(socketMessage, BUFFERSIZE, "CHECKPOINT: %s:%d %s()\n", __FILE__, __LINE__, __FUNCTION__);
  ...
  */
  ...
  snprintf(socketMessage, BUFFERSIZE, "Submitting rhizome bundle: manifest len=%d, body len=%d\n",
           manifest_length, body_length);
  send_all(socketMessage);
  ...
  if (result_code < 200 || result_code > 202)
  {
    ...
    snprintf(socketMessage, BUFFERSIZE, "POST bundle to rhizome failed: http result = %d\n", result_code);
    send_all(socketMessage);
    if (debug_insert)
    {
      char filename[1024];
  ...
  else
  {
    ...
    snprintf(socketMessage, BUFFERSIZE, "http result code = %d\n", result_code);
    send_all(socketMessage);
