...
int dump_bytes(FILE *f,char *msg, unsigned char *bytes, int length)
{
  ...
  //fprintf(f,"%s:\n", msg);
  snprintf(socketMessage, BUFFERSIZE, "%s:\n", msg);
  for (int i = 0; i < length; i += 16)
  {
    snprintf(socketMessage, BUFFERSIZE,"%s%04X: ", socketMessage, i);
    for (int j = 0; j < 16; j++)
      if (i + j < length){
        snprintf(socketMessage, BUFFERSIZE,"%s %02X",socketMessage, bytes[i + j]);
        //fprintf(f," %02X", bytes[i + j]);
      }
    //fprintf(f,"  ");
    snprintf(socketMessage, BUFFERSIZE,"%s  ",socketMessage);
    ...
    for (int j = 0; j < 16; j++)
    {
      ...
      if (c > 0x7d)
        c = '.';
      //fprintf(f,"%c", c);
       snprintf(socketMessage, BUFFERSIZE,"%s%c",socketMessage,c);
    }
    //fprintf(f,"\n");
    snprintf(socketMessage, BUFFERSIZE, "%s\n", socketMessage);
  }
