...
int set_nonblock(int fd)
{
  ...
  {
    perror("set_nonblock: fcntl(n,F_GETFL,NULL)");
    return -1;
  }
  if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1)
  {
    perror("set_nonblock: fcntl(n,F_SETFL,n|O_NONBLOCK)");
    return -1;
  }
  return 0;
}
...
int serial_setup_port_with_speed(int fd, int speed)
{
  ...
  */
  snprintf(socketMessage, BUFFERSIZE, "Serial port settings before tcsetaddr: c=%08x, i=%08x, o=%08x, l=%08x\n",
           (unsigned int)t.c_cflag, (unsigned int)t.c_iflag,
           (unsigned int)t.c_oflag, (unsigned int)t.c_lflag);
  ...
  /*fprintf(stderr,"Serial port settings attempting ot be set: c=%08x, i=%08x, o=%08x, l=%08x\n",
	  ...
	  (unsigned int)t.c_oflag,(unsigned int)t.c_lflag);
  */
  snprintf(socketMessage, BUFFERSIZE, "Serial port settings attempting ot be set: c=%08x, i=%08x, o=%08x, l=%08x\n",
           (unsigned int)t.c_cflag, (unsigned int)t.c_iflag,
           (unsigned int)t.c_oflag, (unsigned int)t.c_lflag);
  send_all(socketMessage);
  ...
  /*fprintf(stderr,"Serial port settings after tcsetaddr: c=%08x, i=%08x, o=%08x, l=%08x\n",
	  ...
	  (unsigned int)t.c_oflag,(unsigned int)t.c_lflag);
  */
  snprintf(socketMessage, BUFFERSIZE, "Serial port settings after tcsetaddr: c=%08x, i=%08x, o=%08x, l=%08x\n",
           (unsigned int)t.c_cflag, (unsigned int)t.c_iflag,
           (unsigned int)t.c_oflag, (unsigned int)t.c_lflag);
  send_all(socketMessage);
...
int serial_setup_port(int fd)
{
  ...
  */
  ...
  snprintf(socketMessage, BUFFERSIZE, "Attempting to detect radio type...\n");
  send_all(socketMessage);
  for (int i = 0; radio_types[i].id >= 0; i++)
  {
    ...
    */
    snprintf(socketMessage, BUFFERSIZE, "Autodetecting radio type #%d (%s, %s)\n",
             i, radio_types[i].name, radio_types[i].description);
    send_all(socketMessage);
    if (radio_types[i].autodetect(fd) == 1)
    {
      ...
      */
      snprintf(socketMessage, BUFFERSIZE, "Radio is a %s (%s)\n",
               radio_types[i].name, radio_types[i].description);
      send_all(socketMessage);
      return 0;
    }
  ...
  snprintf(socketMessage, BUFFERSIZE, "Failed to detect type of connected radio.\n");
  send_all(socketMessage);
  return -1;
}
