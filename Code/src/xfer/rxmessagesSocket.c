                ...
                char *prefix, char *servald_server, char *credential)
{
  ...
  */
  ...
  if (debug_pieces)
  {
    ...
    snprintf(socketMessage, BUFFERSIZE, "Decoding message #%d from %s*, length = %d:\n",
             msg_number, peer_prefix, len);
  ...
  if (!p)
  {
    ...
    snprintf(socketMessage, BUFFERSIZE, "Registering peer %s*\n", p->sid_prefix);
    send_all(socketMessage);
  ...
  while (offset < len)
  {
    ...
    if (message_handlers[msg[offset]])
    {
      if (debug_pieces){
        ...
        memset(socketMessage, 0, BUFFERSIZE);
        snprintf(socketMessage, BUFFERSIZE,
                "### %s : Calling message handler for type '%c' @ offset 0x%x\n",
                timestamp_str(), msg[offset], offset);
        send_all(socketMessage);
      }
      ...
      else
      {
        if (debug_pieces){
          ...
          memset(socketMessage, 0, BUFFERSIZE);
          snprintf(socketMessage, BUFFERSIZE,
                "### %s : Handler consumed %d packet bytes.\n",
                timestamp_str(), advance);
          send_all(socketMessage);
        }
